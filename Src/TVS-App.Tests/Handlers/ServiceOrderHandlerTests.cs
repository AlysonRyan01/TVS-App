...
public class ServiceOrderHandlerTests
{
    ...
    public async Task deve_retornar_verdadeiro_se_a_ordem_de_servico_for_criada()
    {
        try
        {
            ...
            var createResult = await _serviceOrderHandler.CreateServiceOrderAsync(createServiceOrderCommand);
    ...
    public async Task deve_retornar_verdadeiro_se_obter_todas_as_ordens_de_servico_corretamente()
    {
        try
        {
            var getAllServiceOrdersResult = await _serviceOrderHandler.GetAllServiceOrdersAsync(1, 25);
    ...
    public async Task deve_retornar_verdadeiro_se_todas_as_listas_estiverem_com_o_service_order_status_correto()
    {
        try
        {
            var allServiceOrderResponse = await _serviceOrderHandler.GetAllServiceOrdersAsync(1, 25);
            ...
            for (int i = 0; i < 2; i++)
            {
                allserviceOrder[i]?.AddEstimate("placa", 200m, 300m, Domain.Enums.ERepairResult.Repair);
            ...
            var estimatedServiceOrders = await _serviceOrderHandler.GetWaitingResponseAsync(1, 25);
            ...
            var awaitingPartServiceOrders = await _serviceOrderHandler.GetWaitingPartsAsync(1, 25);
            ...
            var awaitingPickUpServiceOrders = await _serviceOrderHandler.GetWaitingPickupAsync(1, 25);
            ...
            var getDeliveredServiceOrders = await _serviceOrderHandler.GetDeliveredAsync(1, 25);
            ...
            var getPendingEstimatesServiceOrders = await _serviceOrderHandler.GetPendingEstimatesAsync(1, 25);
